...
# Extract baseline name from solution path (e.g., "eplb/human_SOTA" -> "human_SOTA")
...
echo "[run_evaluator] Running EPLB evaluator..." >&2
...
try:
    ...
    if 'balancedness_score_gpu' in results:
        print(f"  Balancedness Score GPU: {results['balancedness_score_gpu']:.4f}", file=sys.stderr)
    if 'balancedness_score_expert' in results:
        print(f"  Balancedness Score Expert: {results['balancedness_score_expert']:.4f}", file=sys.stderr)
    if 'times_algorithm' in results:
        print(f"  Algorithm Time: {results['times_algorithm']:.6f}s", file=sys.stderr)
    if 'error' in results:
