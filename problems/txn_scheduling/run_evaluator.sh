...
# run_evaluator.sh for txn_scheduling
...
# Extract baseline name from solution path (e.g., "txn_scheduling/human_SOTA" -> "human_SOTA")
...
echo "[run_evaluator] Running TXN_SCHEDULING evaluator..." >&2
...
try:
    ...
    if 'total_makespan' in results:
        print(f"  Total Makespan: {results['total_makespan']:.2f}", file=sys.stderr)
    if 'baseline_makespan' in results:
        print(f"  Baseline Makespan: {results['baseline_makespan']:.2f}", file=sys.stderr)
    if 'time_taken' in results:
        print(f"  Time Taken: {results['time_taken']:.4f}s", file=sys.stderr)
    if 'error' in results:
