...
def evaluate_search_algorithm(program_module: ModuleType, config_files: List[Path], num_vms: int) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        os.chdir(temp_dir)
        try:
            cost_csv = RESOURCES / "datasets" / "profiles" / "cost.csv"
            throughput_csv = RESOURCES / "datasets" / "profiles" / "throughput.csv"
    ...
    LOWER_COST = 1199.00   # worst
    UPPER_COST = 626.24    # best

    # Clamp total_cost into the valid scoring range
    cost_clamped = max(min(total_cost, LOWER_COST), UPPER_COST)

    # Normalize so UPPER_COST => 100, LOWER_COST => 0
    normalized_cost = (LOWER_COST - cost_clamped) / (LOWER_COST - UPPER_COST)
    score = normalized_cost * 100
    combined_score = normalized_cost
