...
DATASETS_DIR="$BASE_DIR/datasets/cant_be_late"

mkdir -p "$DATASETS_DIR"

echo "[cant_be_late download] Checking for real_traces dataset..."

# Check if dataset already exists
if compgen -G "$DATASETS_DIR/real/ddl=search+task=48+overhead=*/real/*/traces/random_start/*.json" >/dev/null 2>&1; then
  echo "[cant_be_late download] Dataset already exists at $DATASETS_DIR"
  exit 0
fi
...
# Extract dataset
echo "[cant_be_late download] Extracting dataset..."
...
echo "[cant_be_late download] Dataset ready at $DATASETS_DIR"
