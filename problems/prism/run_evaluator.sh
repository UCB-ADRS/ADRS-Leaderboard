...
# Extract baseline name from solution path (e.g., "prism/human_SOTA" -> "human_SOTA")
...
echo "[run_evaluator] Running PRISM evaluator..." >&2
...
try:
    ...
    if 'avg_kvpr' in results:
        print(f"  Average KVPR: {results['avg_kvpr']:.4f}", file=sys.stderr)
    if 'baseline_kvpr' in results:
        print(f"  Baseline KVPR: {results['baseline_kvpr']:.4f}", file=sys.stderr)
    if 'success_rate' in results:
        print(f"  Success Rate: {results['success_rate']:.2%}", file=sys.stderr)
    if 'error' in results:
