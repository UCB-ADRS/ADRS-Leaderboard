...
# run_evaluator.sh for telemetry_repair
...
# Extract baseline name from solution path (e.g., "telemetry_repair/human_SOTA" -> "human_SOTA")
...
echo "[run_evaluator] Running TELEMETRY_REPAIR evaluator..." >&2
...
try:
    ...
    if 'counter_repair_accuracy' in results:
        print(f"  Counter Repair Accuracy: {results['counter_repair_accuracy']:.4f}", file=sys.stderr)
    if 'status_repair_accuracy' in results:
        print(f"  Status Repair Accuracy: {results['status_repair_accuracy']:.4f}", file=sys.stderr)
    if 'confidence_calibration' in results:
        print(f"  Confidence Calibration: {results['confidence_calibration']:.4f}", file=sys.stderr)
    if 'error' in results:
