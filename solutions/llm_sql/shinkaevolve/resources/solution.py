...
class Evolved(Algorithm):
    ...
    def column_recursion(self, result_df, max_value, grouped_rows, row_stop, col_stop, early_stop):
        ...
        if not result_df.empty:
            ...
            for _, group in grouped_result_df:
                ...
                # Update the group with the reordered columns while steering clear of chained-assignment
                if dependent_cols:
                    target_cols = group.columns[length_of_settle_cols:]
                else:
                    target_cols = group.columns[1:]
                # Work on a copy to avoid chained-assignment warnings and dtype issues
                group_copy = group.copy()
                group_copy[target_cols] = group_copy[target_cols].astype(object)
                group_copy.loc[:, target_cols] = reordered_group_remainder.values
                # Directly assign the updated columns back to the parent DataFrame to avoid
                # down-casting warnings that arise from the `update` method.
                result_df[target_cols] = result_df[target_cols].astype(object)
                result_df.loc[group_copy.index, target_cols] = group_copy[target_cols]
                break
    ...
    def reorder(
        ...
        # Use assign to avoid chained-assignment warnings
        df = df.assign(original_index=range(len(df)))
